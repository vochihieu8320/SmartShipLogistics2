...
export const API_BASE_URL = "https://f155-103-249-22-81.ngrok-free.app/api/v1";
...
async function loadApiConfig() {
  try {
    ...
    if (response.ok) {
      ...
      ACTIVE_API_URL = USE_EXTERNAL_API
        ? config.externalApiUrl || API_BASE_URL
        : LOCAL_API_URL;

      console.log(
        `[API] Using ${USE_EXTERNAL_API ? "external" : "local"} API: ${ACTIVE_API_URL}`,
      );
    } else {
